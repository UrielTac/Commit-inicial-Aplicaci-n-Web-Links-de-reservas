...
export function ItemsPreview({ 
  ...
  useEffect(() => {
    setQuantities(state.items.selectedItems);
  ...
  const handleQuantityChange = useCallback((itemId: string, change: number) => {
    setQuantities(prev => {
      ...
      const newQuantity = Math.max(0, existingQuantity + change);
                      ...
                      filteredItems.map((item, index) => {
                        ...
                        return (
                          <motion.button
                            ...
                            onClick={() => !isOutOfStock && handleQuantityChange(item.id, 1)}
                          >
                            <AnimatePresence>
                              {isSelected && (
                                <motion.div
                            ...
                            <div className="p-2.5 relative z-10">
                              ...
                              <div className="flex items-center justify-between mt-2">
                                ...
                                <div onClick={(e) => e.stopPropagation()}>
                                  <QuantitySelector 
                                    value={quantity}
                                    onChange={(value) => handleQuantityChange(item.id, value - quantity)}
                                    theme={theme}
                                    min={0}
